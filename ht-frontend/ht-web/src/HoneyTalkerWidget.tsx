...
const themes = {
  economics: {
    primary: '#10b981',      // Green (matches NEU)
    secondary: '#059669',
...
const HoneyTalkerWidget: React.FC<HoneyTalkerWidgetProps> = ({
  theme = 'default',
  ...
  const chatService = useRef(new ChatService(apiUrl, orgId));
  const currentTheme = themes[theme as keyof typeof themes];
  ...
  const handleSendMessage = async (message: string) => {
    ...
    try {
      const response = await chatService.current.sendMessage(message, theme);
